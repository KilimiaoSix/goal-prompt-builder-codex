...
Loaded by `goal-prompt-builder` after the user picks a scenario in Step 2. Each section provides a fillable skeleton plus the design rationale, so Codex knows why the skeleton looks the way it does and can adapt intelligently when the user's actual case does not fit perfectly.
